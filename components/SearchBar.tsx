	...
	return (
		<>
			...
			<PortalModal isOpen={open} onClose={handleModalToggle}>
				<div className="bg-background rounded-lg shadow-lg p-3 w-full md:w-[25vw] md:border md:border-secondary h-screen md:h-[75vh] md:max-h-[90vh] overflow-y-auto flex flex-col">
					...
					<input
						...
						autoFocus
						className="w-full bg-transparent text-black dark:text-white placeholder-gray-400 dark:placeholder-gray-400 placeholder:text-md text-lg px-3 py-3 rounded-md focus:outline-none focus:ring-0 focus:border-accent border-gray-400"
					/>
					...
					{isListVisible && (
						<div className="mt-2 rounded-md bg-background text-black dark:text-white max-h-[400px] w-full">
							{searchValue.trim() === '' && searchHistory.length > 0 && (
								<div className="px-3 py-3 text-md font-bold flex justify-between items-center">
							...
							{searchHistory.map((item, index) => (
								<div
									...
									className={`group px-3 py-3 cursor-pointer transition text-sm 
							...
							{(playerSearchList.length > 0 || dareSearchList.length > 0) && (
								<>
									{playerSearchList.length > 0 && <div className="px-3 py-3 text-xs font-bold text-secondary">Players</div>}
									{playerSearchList.map((player) => (
										<div
											key={player.id}
											onClick={() => handleListPlayerItemClick(player.name, player.resolver?.addr?.id)}
											className="px-3 py-3 cursor-pointer hover:bg-secondary/20 transition text-sm"
										>
											<div>{player.name}</div>
									...
									{/* Dare Search List */}
									{dareSearchList.length > 0 && <div className="px-3 py-3 text-xs font-bold text-secondary">Dares</div>}
							...
							{searchValue.trim() !== '' && playerSearchList.length === 0 && dareSearchList.length === 0 && (
								<div className="px-3 py-3 text-xs text-muted text-center">No players or dares found.</div>
							)}

							{/* Trending Players */}
							{searchValue.trim() === '' && (
								<div className="flex flex-col mt-8 px-3 py-3">
